...
10. If the lane is premium, call `list_svg_models`, `estimate_motion_cost`, then `generate_premium_svg_asset`. Use `vectorize_asset` for existing raster assets. Use `vectorize_video` to turn a short video (mp4/mov/webm) into a vector flipbook animation — fully local, no AI credits beyond staging. The receipt includes a `rigProposal` when cross-frame tracking found multiple moving parts; surface it for review before animating. When the video's motion reads as smooth translation rather than flipbook cuts, call `motion_to_curves` with the tracked parts to stage eased translate tracks over persistent per-part layers instead.
